...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if length(V)~=0
        OE=ObserveEvidence(F,E);% Obtener evidencias
        for i=1:length(OE) % Normalizar
            OE(i).val=OE(i).val./sum(OE(i).val);
        end
        Joint=ComputeJointDistribution(OE);% JoointDistribution
        M=FactorMarginalization(Joint,setdiff(Joint.var,V));% Marginalizacion
        M.val=M.val./sum(M.val);% Renormalizar
   end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
